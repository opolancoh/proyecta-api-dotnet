...
public class AuthWebApplicationFactory : WebApplicationFactory<Program>, IDisposable
{
    ...
    public async Task<(ApplicationUser, string)> CreateUserAsync()
    {
    ...
    private async Task<ApplicationUser> CreateUserAsync(string firstName, string lastName, string username,
        string password)
    {
    ...
    private async Task AssignUserToRoleAsync(ApplicationUser user, string role)
    {
        var userManager = _scope.ServiceProvider.GetRequiredService<UserManager<ApplicationUser>>();
